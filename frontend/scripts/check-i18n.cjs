...
const catalog = {
  ...require(path.join(sourceRoot, "i18n", "en.json")),
  ...require(path.join(sourceRoot, "i18n", "app-en.json")),
};
const ignoredFiles = new Set([
  path.join(sourceRoot, "shared", "LanguageSwitcher.tsx"),
  path.join(sourceRoot, "i18n", "index.tsx"),
]);
...
for (const filePath of sourceFiles(sourceRoot)) {
  ...
  const visit = (node) => {
    if (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)) record(node.text, sourceFile, node, "string");
    if (ts.isJsxText(node)) record(node.getText(sourceFile), sourceFile, node, "jsx");
    if (ts.isTemplateExpression(node)) {
      const parts = [node.head.text, ...node.templateSpans.map((span, index) => `{${index + 1}}${span.literal.text}`)];
...
const invalidTargets = Object.entries(catalog).filter(([, target]) => !target.trim() || /[\u3400-\u9fff]/.test(target));
if (missing.size || invalidTargets.length) {
  if (missing.size) {
    console.error(`Missing English translations (${missing.size}):`);
    for (const [location, value] of missing) console.error(`- ${path.relative(projectRoot, location)}: ${JSON.stringify(value)}`);
  }
  if (invalidTargets.length) {
    console.error(`Invalid English translations (${invalidTargets.length}):`);
    for (const [source, target] of invalidTargets) console.error(`- ${JSON.stringify(source)} => ${JSON.stringify(target)}`);
  }
  process.exit(1);
}
