...
import appEnglishCatalog from "./app-en.json";
...
const STORAGE_KEY = "mem_kb_locale";
const CATALOG = { ...englishCatalog, ...appEnglishCatalog } as Record<string, string>;
const SAFE_ATTRIBUTES = ["placeholder", "aria-label", "title", "alt", "data-placeholder", "data-tooltip"] as const;
...
function shouldIgnore(element: Element | null): boolean {
  return Boolean(element?.closest('[data-i18n-ignore], code, pre, script, style, textarea, [contenteditable="true"]'));
}
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const t = useCallback((source: string, values: Record<string | number, string | number> = {}) => {
  ...
  const value = useMemo<I18nContextValue>(() => ({ locale, setLocale, toggleLocale, t }), [locale, setLocale, t, toggleLocale]);
