...
export function NightConsolidation() {
  ...
  return <div className="settings-content night-consolidation">
    ...
    <div className="consolidation-grid">
      <section className="consolidation-config"><h2>任务设置</h2><label><span><Clock3 />执行时间</span><input type="time" value={scheduleTime} onChange={(event) => setScheduleTime(event.target.value)} /></label><fieldset><legend>知识库范围</legend><p>不勾选时覆盖全部可用知识库。</p><div>{workspaces.map((workspace) => <label key={workspace.id}><input type="checkbox" checked={workspaceIds.includes(workspace.id)} onChange={(event) => setWorkspaceIds((current) => event.target.checked ? [...current, workspace.id] : current.filter((id) => id !== workspace.id))} /><span><strong>{workspace.name}</strong><em>{workspace.kind === "image" ? "图片知识" : "文档知识"}</em></span></label>)}</div></fieldset><button className="button secondary" disabled={busy} onClick={() => void save()}><Save size={15} />保存设置</button>{config?.next_run_at ? <p className="next-run">下次执行：{new Date(config.next_run_at).toLocaleString("zh-CN")}</p> : null}</section>
      <section className="consolidation-result"><h2>最近执行</h2>{latest ? <><header><span className={latest.status}>{latest.status === "completed" ? "已完成" : latest.status === "failed" ? "失败" : "执行中"}</span><time>{new Date(latest.started_at).toLocaleString("zh-CN")}</time></header><div className="consolidation-stats"><Stat icon={<ScanSearch />} label="扫描对话" value={latest.conversations_scanned} /><Stat icon={<Link2 />} label="实体关系" value={latest.relations_added} /><Stat icon={<Wrench />} label="修复引用" value={latest.citations_repaired} /><Stat icon={<BookOpenCheck />} label="整理节点" value={latest.structures_organized} /></div>{latest.error ? <p className="form-error">{latest.error}</p> : null}<div className="consolidation-log"><h3>执行日志</h3>{visibleLogs.map((log) => <div key={log.id} className={log.level}><time>{new Date(log.created_at).toLocaleTimeString("zh-CN", { hour12: false })}</time><span>{log.message}</span></div>)}</div></> : <div className="settings-empty"><MoonStar size={30} /><strong>还没有执行记录</strong><span>保存设置并立即运行一次即可验证。</span></div>}</section>
    </div>
  </div>;
}
